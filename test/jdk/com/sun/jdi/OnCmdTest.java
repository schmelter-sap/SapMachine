 ...
 * @test
 * @bug 8205608
 * @summary Test that the oncmd option of the jdwp agent works.
 *
 * @author Ralf Schmelter
 *
 * @library /test/lib
 * @run compile --add-exports java.base/jdk.internal.vm=ALL-UNNAMED -g OnCmdTest.java
 * @run main/othervm --add-exports java.base/jdk.internal.vm=ALL-UNNAMED -agentlib:jdwp=transport=dt_socket,address=localhost:0,oncmd=y,server=y OnCmdTest
 */

public class OnCmdTest {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        OutputAnalyzer output = ProcessTools.executeTestJvm(
                "-agentlib:jdwp=transport=dt_socket,address=any,oncmd=y");
        output.shouldContain("Can only use oncmd with server=y");
        output.shouldHaveExitValue(1);

        output = ProcessTools.executeTestJvm(
                "-agentlib:jdwp=transport=dt_socket,address=any,oncmd=y,onthrow=a,launch=a");
        output.shouldContain("Cannot combine oncmd and launch suboptions");
        output.shouldHaveExitValue(1);
        ...
        output.shouldContain("Debugging has been started initially");
